...
After the first batch of fixes (Agency OS/AI Studio crash, callClaude routing, onboarding render), Netlify stopped picking up new pushes to `main` — the live bundle hash stayed frozen at `index-Ml5ktS5b.js` across several subsequent pushes and 20+ minutes, despite GitHub's `main` branch correctly showing each new commit (verified directly via the GitHub API). Initially suspected `package-lock.json` (newly added this session, and Netlify defaults to the stricter `npm ci` when a lockfile is present) and removed it, but the build still didn't trigger — **that diagnosis turned out to be wrong** (or at least unconfirmed): the owner clarified mid-session that Netlify build credits/minutes are running low, which is the more likely real cause and fits the symptoms better (a quota stop, not a build failure). Correcting the record here rather than leaving a wrong conclusion in place. Given this, pushes to `main` were batched for the rest of the session instead of pushing after every commit, to conserve remaining credits. All commits are correctly on GitHub regardless; Worker deploys are unaffected (separate mechanism via the Cloudflare API, verified live via direct curl throughout).
