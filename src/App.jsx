  ...
  const submit=async()=>{
    ...
    try{
      if(mode==='signup'){
        ...
        if(err)throw err;
        if(data.user)onAuth({name:name||email.split('@')[0],email,id:data.user.id});
      }else{
...
function SettingsPage({user,onUpdateUser,toast,setProspects,setPitches,setProposals,setInvoices}){
  ...
  const[keys,setKeys]=useState({github:'',netlify:'',stripe:''});
  ...
  useEffect(()=>{
    ...
    ['github','netlify','stripe'].forEach(k=>{
      ...
      <div className="settings-grid">
        ...
        <div className="settings-card">
          ...
          <div style={{display:'flex',alignItems:'center',gap:12,padding:'12px 0'}}>
            <div style={{fontFamily:"'Cinzel',serif",fontSize:'1.4rem',fontWeight:700,color:'#c9a84c'}}>Apprentice</div>
            <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:'.58rem',letterSpacing:'2px',textTransform:'uppercase',color:'#7ac89a',padding:'3px 10px',border:'1px solid rgba(122,200,154,.3)',background:'rgba(122,200,154,.06)'}}>FREE</div>
          </div>
          <button className="btn btn-gold btn-sm">Upgrade Plan →</button>
        </div>
