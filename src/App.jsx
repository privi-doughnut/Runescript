      ...
      <div className="agency-layout">
        ...
        <div className="agency-content">
          ...
          {showLibrary&&savedOutputs.length>0&&(
            <div style={{borderTop:'1px solid rgba(201,168,76,.08)',paddingTop:12,marginTop:4}}>
      ...
      <div className="studio-layout">
        ...
        <div className="studio-content">
          ...
          <div style={{marginBottom:10}}><input className="inp" placeholder="Brand voice (optional): e.g. bold, witty, premium…" value={brandVoice} onChange={e=>{setBrandVoice(e.target.value);window.storage.set('rs3_brandvoice',e.target.value).catch(()=>{});}} style={{fontSize:'.78rem'}}/></div><div className="studio-output">
            ...
            {output&&!loading&&(<><div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:".56rem",letterSpacing:"2px",color:"#2e2d3c",textTransform:"uppercase",marginBottom:12}}>Generated Content</div><div className="studio-out-body"><div style={{fontFamily:"'DM Sans',sans-serif",fontSize:".9rem",fontWeight:300,color:"#c9b87a",lineHeight:1.95,whiteSpace:"pre-wrap",letterSpacing:".3px"}}>{output}</div></div><div className="studio-copy-row"><button className="btn btn-ghost btn-sm" onClick={copy}>{copied?"Copied ✓":"Copy All"}</button><button className="btn btn-ghost btn-sm" onClick={()=>setOutput("")}>Clear</button><button className="btn btn-ghost btn-sm" onClick={()=>{if(!output)return;setSavedOutputs(prev=>[{id:uid(),text:output,tool:activeTool?.name||'',savedAt:now()},...prev.slice(0,29)]);toast("Saved to library.","success");}}>Save</button><button className="btn btn-ghost btn-sm" onClick={()=>{if(!output)return;const when=window.prompt('Schedule for when? (e.g. "Tomorrow 9am", "Mon", a date)');if(!when)return;const sched={id:uid(),text:output,tool:activeTool?.name||'',when,createdAt:now()};window.storage.get('rs3_scheduled').then(r=>{let arr=[];if(r?.value)try{arr=JSON.parse(r.value);}catch(e){}arr.unshift(sched);window.storage.set('rs3_scheduled',JSON.stringify(arr.slice(0,50)));}).catch(()=>{window.storage.set('rs3_scheduled',JSON.stringify([sched]));});toast(`Scheduled for ${when}.`,'success');}}>📅 Schedule</button></div></>)}
          </div>
        </div>
      </div>
    </div>
