...
// ── DASHBOARD ──────────────────────────────────────────────────────────────
function DashboardPage({user,prospects,pitches,setPage,siteBuilt,demoMode,setProspects,setPitches,setProposals,setInvoices,setDemoMode,toast}){
...
function CreatorPage({toast}) {
  ...
  const [form, setForm] = useState({name:'',email:'',platform:'Instagram',handle:'',followers:'',why:'',tier:'Archon'});
  const TIERS = [
    {name:'Archon',req:'10K+ followers on any platform',value:'$99/mo free',feats:['Full platform access','Pitch generator','Site builder','Agency OS','AI Studio']},
    {name:'Sovereign',req:'50K+ followers or strong agency audience',value:'$199/mo free',feats:['Everything in Archon','Mobile app builder','Full automation suite','White-label client portal']},
  ];
  const submit = () => {
    if(!form.name||!form.email||!form.handle){toast('Fill in name, email, and handle.','error');return;}
    setApps(prev=>[...prev,{...form,id:uid(),status:'Under Review',submittedAt:now()}]);
    setForm({name:'',email:'',platform:'Instagram',handle:'',followers:'',why:'',tier:'Archon'});
    toast('Application submitted! We review within 48 hours.','success');
    setTab('status');
  };
  return (
    <div>
      <div className="sh"><div><div className="sh-title">Creator Program</div><div className="sh-sub">Free plan access in exchange for authentic promotion</div></div></div>
      <div className="creator-hero">
        <div className="creator-hero-rune">ᚷ</div>
        <div className="creator-hero-title">The Rune Script Creator Program</div>
        <p className="creator-hero-sub">We give you a free Archon or Sovereign plan. You build something real with it and share your experience with your audience. No scripts. No forced content. Just honest results.</p>
      </div>
      <div className="creator-tiers" style={{marginBottom:24}}>
        {TIERS.map((t,i)=>(
          <div key={i} className="creator-tier" onClick={()=>setForm(f=>({...f,tier:t.name}))} style={{border:`1px solid ${form.tier===t.name?'rgba(201,168,76,.35)':'rgba(201,168,76,.08)'}`,background:form.tier===t.name?'rgba(201,168,76,.05)':'#0d0d18',cursor:'pointer'}}>
            <div className="creator-tier-name">{t.name}</div>
            <div style={{fontFamily:"'Cinzel',serif",fontSize:'1rem',fontWeight:700,color:'#7ac89a',marginBottom:6}}>{t.value}</div>
            <div className="creator-tier-req" style={{marginBottom:12}}>{t.req}</div>
            <ul className="creator-tier-features">
              {t.feats.map(f=><li key={f} className="creator-tier-feat"><span className="creator-tier-feat-ic">ᚱ</span>{f}</li>)}
            </ul>
            {form.tier===t.name && <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:'.58rem',letterSpacing:'2px',color:'#c9a84c',textTransform:'uppercase'}}>Selected ✦</div>}
          </div>
        ))}
      </div>
      <div className="tabs">
        <button className={`tab${tab==='apply'?' on':''}`} onClick={()=>setTab('apply')}>Apply</button>
        <button className={`tab${tab==='status'?' on':''}`} onClick={()=>setTab('status')}>My Applications ({apps.length})</button>
        <button className={`tab${tab==='faq'?' on':''}`} onClick={()=>setTab('faq')}>FAQ</button>
      </div>
      {tab==='apply' && (
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(260px,1fr))',gap:12}}>
          <div className="card">
            <div className="card-title" style={{marginBottom:4}}>Your Info</div>
            <div className="card-sub" style={{marginBottom:16}}>Tell us about yourself</div>
            <div className="field"><label>Full Name</label><input className="inp" placeholder="Jordan Martinez" value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))}/></div>
            <div className="field"><label>Email</label><input className="inp" type="email" placeholder="jordan@example.com" value={form.email} onChange={e=>setForm(f=>({...f,email:e.target.value}))}/></div>
            <div className="field"><label>Primary Platform</label>
              <select className="inp" value={form.platform} onChange={e=>setForm(f=>({...f,platform:e.target.value}))}>
                {['Instagram','TikTok','YouTube','LinkedIn','Twitter/X','Facebook','Podcast'].map(p=><option key={p} value={p}>{p}</option>)}
              </select>
            </div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
              <div className="field"><label>Handle / Username</label><input className="inp" placeholder="@yourhandle" value={form.handle} onChange={e=>setForm(f=>({...f,handle:e.target.value}))}/></div>
              <div className="field"><label>Follower Count</label><input className="inp" placeholder="12,500" value={form.followers} onChange={e=>setForm(f=>({...f,followers:e.target.value}))}/></div>
            </div>
          </div>
          <div className="card">
            <div className="card-title" style={{marginBottom:4}}>Your Case</div>
            <div className="card-sub" style={{marginBottom:16}}>Make us want to work with you</div>
            <div className="field"><label>Plan You Want</label>
              <select className="inp" value={form.tier} onChange={e=>setForm(f=>({...f,tier:e.target.value}))}>
                <option value="Archon">Archon ($99/mo) — 10K+ followers</option>
                <option value="Sovereign">Sovereign ($199/mo) — 50K+ followers</option>
              </select>
            </div>
            <div className="field"><label>Why are you a good fit? What would you make?</label>
              <textarea className="inp" rows={6} placeholder="Tell us about your audience, what kind of content you make, and how you'd use Rune Script authentically…" value={form.why} onChange={e=>setForm(f=>({...f,why:e.target.value}))}/>
            </div>
            <button className="btn btn-gold btn-full" onClick={submit}>Submit Application →</button>
            <p style={{fontSize:'.72rem',fontWeight:300,color:'#3a3848',lineHeight:1.6,marginTop:10}}>We review every application within 48 hours. If approved, you get immediate access. No contracts — just make honest content.</p>
          </div>
        </div>
      )}
      {tab==='status' && (
        apps.length===0?(
          <div className="empty"><div className="empty-rune">ᚷ</div><div className="empty-title">No applications yet</div><div className="empty-sub">Submit your application above.</div></div>
        ):(
            ...
            {apps.map(a=>(
              <div key={a.id} className="app-card">
                <div style={{flex:1}}>
                  ...
                  <div className="app-meta">{a.platform} · {a.handle} · {a.followers} followers</div>
                  <div className="app-handle">{a.tier} plan · Applied {a.submittedAt}</div>
                </div>
                <span className={`badge ${a.status==='Approved'?'b-green':a.status==='Rejected'?'b-red':'b-gold'}`}>{a.status}</span>
              </div>
            ))}
          </div>
        )
      ...
      {tab==='faq' && (
        <div className="card">
          {[
            {q:"Do I have to post a certain number of times?",a:"No minimum post requirement. We just ask that you use the platform genuinely and share your experience if you find it valuable. Quality over quantity."},
            {q:"Can I keep the plan if I stop posting?",a:"We review partnerships quarterly. If you haven't posted anything in 90 days we'll reach out. If the plan is just sitting unused we'll need to reassign it."},
            {q:"What kind of content do you want?",a:"Anything authentic — tutorials, results, walkthroughs, comparisons, honest reviews. We specifically don't want scripted promotions or fake results."},
            {q:"Can I apply for a friend or client?",a:"No, applications must be for your own account. We check that the handle matches the applicant."},
            {q:"What if I get approved and then hate the product?",a:"Be honest about it. Genuine critical content is more valuable to us than forced positivity. We just ask that you give it a real try first."},
          ].map((f,i)=>(
            <div key={i} style={{paddingBottom:16,marginBottom:16,borderBottom:'1px solid rgba(201,168,76,.06)'}}>
              <div style={{fontFamily:"'Cinzel',serif",fontSize:'.88rem',fontWeight:600,color:'#ddd8ce',marginBottom:8}}>{f.q}</div>
              <p style={{fontSize:'.82rem',fontWeight:300,color:'#5a5868',lineHeight:1.8}}>{f.a}</p>
            </div>
          ))}
        </div>
      )}
...
export default function RuneScript(){
  ...
  const PAGES={
    dashboard:<DashboardPage user={user} prospects={prospects} pitches={pitches} setPage={setPage} siteBuilt={siteBuilt} demoMode={demoMode} setProspects={setProspects} setPitches={setPitches} setProposals={setProposals} setInvoices={setInvoices} setDemoMode={setDemoMode} toast={toast}/>,
