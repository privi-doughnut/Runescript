  ...
  const scan=async()=>{
    ...
    try{
      ...
      if(placesKey){
        ...
        const raw=await callClaude(prompt,Math.min(400*(Number(count)||10)+400,8000));
        const parsed=JSON.parse(raw.replace(/```json|```/g,'').trim());
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
          ...
          <div className="field" style={{margin:0}}>
            ...
            <div style={{display:"flex",gap:6,marginTop:6}}>
              ...
              <button type="button" className="btn btn-ghost btn-sm" title="Randomize" onClick={()=>setCat(BUSINESS_TYPES[Math.floor(Math.random()*BUSINESS_TYPES.length)])}>🎲 Surprise me</button>
            </div>
          </div>
        </div>
