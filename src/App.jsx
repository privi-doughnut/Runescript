...
function ProspectDetails({p, onOwnerChange, compact}) {
  ...
  const [drafts, setDrafts] = useState({});      // review index -> AI-drafted reply
  const [draftingIdx, setDraftingIdx] = useState(null);
  const [copiedIdx, setCopiedIdx] = useState(null);
  const draftReply = async (i, r) => {
    setDraftingIdx(i);
    try {
      const reply = await callClaude(`You are ${p.ownerName||'the owner'} of "${p.name}", a ${p.category||'local business'}. Write a warm, professional, specific reply to this Google review (2-3 sentences, no hashtags, sound human not corporate). Review (${r.rating||5} stars): "${r.text}". Return only the reply text.`, 300);
      setDrafts(d => ({...d, [i]: reply.trim()}));
    } catch(e) { setDrafts(d => ({...d, [i]: 'Could not draft a reply — try again.'})); }
    setDraftingIdx(null);
  };
            ...
            <div style={{marginTop:6,display:'flex',flexDirection:'column',gap:6}}>
              {p.topReviews.map((r,i)=>(
                <div key={i} style={{fontSize:'.72rem',color:'#7a7888',lineHeight:1.5,padding:'6px 8px',background:'rgba(201,168,76,.03)',border:'1px solid rgba(201,168,76,.06)'}}>
                  ...
                  {!drafts[i]&&<button className="btn btn-ghost btn-xs" style={{marginTop:6}} disabled={draftingIdx===i} onClick={()=>draftReply(i,r)}>{draftingIdx===i?'Drafting…':'✍ Draft a reply'}</button>}
                  {drafts[i]&&(
                    <div style={{marginTop:6,padding:'6px 8px',background:'rgba(122,200,154,.06)',border:'1px solid rgba(122,200,154,.15)'}}>
                      <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:'.5rem',letterSpacing:'1px',color:'#7ac89a',textTransform:'uppercase',marginBottom:3}}>Suggested reply</div>
                      <div style={{color:'#9a96a2'}}>{drafts[i]}</div>
                      <button className="btn btn-ghost btn-xs" style={{marginTop:5}} onClick={()=>{navigator.clipboard.writeText(drafts[i]);setCopiedIdx(i);setTimeout(()=>setCopiedIdx(null),1500);}}>{copiedIdx===i?'Copied ✓':'Copy'}</button>
                    </div>
                  )}
                </div>
              ))}
...
const SECTION_SNIPPETS = [
  ...
  // E-commerce: each product gets its own real Stripe Checkout session via
  // the Worker's /create-invoice-payment endpoint (no persistent cart —
  ...
  // The sections below are all self-contained static HTML (no external
  // services, no single-quotes/apostrophes so they survive as JS strings) —
  // safe to drop onto any client site. Forms post via mailto so they work
  // with zero backend.
  {label:'FAQ',tag:'section',html:'<section style="padding:70px 24px;max-width:760px;margin:0 auto"><h2 style="font-size:1.8rem;text-align:center;margin-bottom:32px">Frequently Asked Questions</h2><div style="padding:18px 0;border-bottom:1px solid #e5e5e5"><h3 style="margin-bottom:8px;font-size:1.05rem">What areas do you serve?</h3><p style="color:#555;line-height:1.6">We proudly serve the entire metro area and surrounding communities. Contact us to confirm coverage for your location.</p></div><div style="padding:18px 0;border-bottom:1px solid #e5e5e5"><h3 style="margin-bottom:8px;font-size:1.05rem">How do I get a quote?</h3><p style="color:#555;line-height:1.6">Call us or fill out the contact form and we will get back to you within one business day with a free, no-obligation estimate.</p></div><div style="padding:18px 0;border-bottom:1px solid #e5e5e5"><h3 style="margin-bottom:8px;font-size:1.05rem">Are you licensed and insured?</h3><p style="color:#555;line-height:1.6">Yes. We are fully licensed, bonded, and insured, so you are protected on every job.</p></div></section>'},
  {label:'Team',tag:'section',html:'<section style="padding:70px 24px;background:#fafafa"><h2 style="font-size:1.8rem;text-align:center;margin-bottom:36px">Meet the Team</h2><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:28px;max-width:900px;margin:0 auto;text-align:center">{TEAM}</div></section>'.replace('{TEAM}',['Alex Rivera:Founder','Jordan Lee:Lead Specialist','Sam Park:Customer Success'].map(function(m){var p=m.split(':');return '<div><div style="width:120px;height:120px;border-radius:50%;background:#e0e0e0;margin:0 auto 14px"></div><h3 style="margin-bottom:4px">'+p[0]+'</h3><p style="color:#888;font-size:.9rem">'+p[1]+'</p></div>';}).join(''))},
  {label:'Menu',tag:'section',html:'<section style="padding:70px 24px;max-width:700px;margin:0 auto"><h2 style="font-size:1.8rem;text-align:center;margin-bottom:36px">Menu</h2><div style="display:flex;justify-content:space-between;padding:14px 0;border-bottom:1px dashed #ddd"><div><strong>House Special</strong><div style="color:#888;font-size:.9rem">Our signature dish, made fresh daily</div></div><strong>$14</strong></div><div style="display:flex;justify-content:space-between;padding:14px 0;border-bottom:1px dashed #ddd"><div><strong>Garden Salad</strong><div style="color:#888;font-size:.9rem">Seasonal greens with house dressing</div></div><strong>$9</strong></div><div style="display:flex;justify-content:space-between;padding:14px 0;border-bottom:1px dashed #ddd"><div><strong>Classic Entree</strong><div style="color:#888;font-size:.9rem">A customer favorite</div></div><strong>$18</strong></div></section>'},
  {label:'Booking',tag:'section',html:'<section style="padding:70px 24px;text-align:center;background:#f5f5f5"><h2 style="font-size:1.8rem;margin-bottom:12px">Book an Appointment</h2><p style="color:#555;max-width:520px;margin:0 auto 24px;line-height:1.6">Pick a time that works for you. Send us your preferred date and time and we will confirm within a few hours.</p><form action="mailto:you@example.com" method="post" enctype="text/plain" style="max-width:420px;margin:0 auto;display:flex;flex-direction:column;gap:12px"><input name="name" placeholder="Your name" style="padding:12px;border:1px solid #ccc;border-radius:6px" required><input name="date" type="date" style="padding:12px;border:1px solid #ccc;border-radius:6px" required><input name="phone" placeholder="Phone number" style="padding:12px;border:1px solid #ccc;border-radius:6px"><button type="submit" style="padding:14px;background:#222;color:#fff;border:none;border-radius:6px;cursor:pointer">Request Booking</button></form></section>'},
  {label:'Newsletter',tag:'section',html:'<section style="padding:60px 24px;text-align:center;background:#222;color:#fff"><h2 style="font-size:1.6rem;margin-bottom:10px">Stay in the Loop</h2><p style="opacity:.8;max-width:460px;margin:0 auto 22px">Join our list for tips, offers, and updates. No spam, unsubscribe anytime.</p><form action="mailto:you@example.com" method="post" enctype="text/plain" style="max-width:440px;margin:0 auto;display:flex;gap:10px"><input name="email" type="email" placeholder="you@email.com" style="flex:1;padding:13px;border:none;border-radius:6px" required><button type="submit" style="padding:13px 24px;background:#fff;color:#222;border:none;border-radius:6px;cursor:pointer;font-weight:600">Subscribe</button></form></section>'},
  {label:'Map',tag:'section',html:'<section style="padding:0"><iframe title="Location map" width="100%" height="360" style="border:0;display:block" loading="lazy" src="https://www.google.com/maps?q=New+York&output=embed"></iframe></section>'},
  {label:'Video',tag:'section',html:'<section style="padding:70px 24px;text-align:center"><h2 style="font-size:1.8rem;margin-bottom:28px">See Us in Action</h2><div style="max-width:820px;margin:0 auto;aspect-ratio:16/9"><iframe title="Video" width="100%" height="100%" style="border:0" loading="lazy" allowfullscreen src="https://www.youtube.com/embed/dQw4w9WgXcQ"></iframe></div></section>'},
  {label:'Loyalty',tag:'section',html:'<section style="padding:70px 24px;text-align:center;background:#fffbea"><h2 style="font-size:1.8rem;margin-bottom:10px">Rewards Program</h2><p style="color:#8a7a3a;max-width:520px;margin:0 auto 28px">Earn a stamp with every visit. Collect 10 and your next one is on us.</p><div style="display:flex;gap:12px;justify-content:center;flex-wrap:wrap;max-width:520px;margin:0 auto">{STAMPS}</div></section>'.replace('{STAMPS}',Array.from({length:10}).map(function(_,i){return '<div style="width:52px;height:52px;border-radius:50%;border:2px solid #d8b84a;display:flex;align-items:center;justify-content:center;color:#d8b84a;font-weight:700">'+(i+1)+'</div>';}).join(''))},
  {label:'Stats',tag:'section',html:'<section style="padding:60px 24px;background:#111;color:#fff"><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:24px;max-width:900px;margin:0 auto;text-align:center"><div><div style="font-size:2.4rem;font-weight:800">15+</div><div style="opacity:.7;margin-top:4px">Years Experience</div></div><div><div style="font-size:2.4rem;font-weight:800">2,000+</div><div style="opacity:.7;margin-top:4px">Happy Customers</div></div><div><div style="font-size:2.4rem;font-weight:800">4.9</div><div style="opacity:.7;margin-top:4px">Average Rating</div></div><div><div style="font-size:2.4rem;font-weight:800">24/7</div><div style="opacity:.7;margin-top:4px">Support</div></div></div></section>'},
  // Floating live-chat bubble: opens a contact panel that emails the business
  // (mailto, zero backend). Toggle logic is in a <script> using only double
  // quotes so it survives as a single-quoted JS string.
  {label:'Live Chat',tag:'section',html:'<div id="rs-chat" style="position:fixed;bottom:20px;right:20px;z-index:9999;font-family:sans-serif"><div id="rs-chat-panel" style="display:none;width:300px;background:#fff;border:1px solid #ddd;border-radius:12px;box-shadow:0 10px 34px rgba(0,0,0,.18);overflow:hidden;margin-bottom:12px"><div style="background:#222;color:#fff;padding:14px 16px;font-weight:600">Chat with us</div><div style="padding:16px"><p style="color:#555;margin-bottom:12px;font-size:.9rem">Hi there! Leave your phone or email and we will get right back to you.</p><form action="mailto:you@example.com" method="post" enctype="text/plain" style="display:flex;flex-direction:column;gap:8px"><input name="contact" placeholder="Phone or email" style="padding:10px;border:1px solid #ccc;border-radius:6px"><textarea name="message" placeholder="How can we help?" rows="3" style="padding:10px;border:1px solid #ccc;border-radius:6px"></textarea><button type="submit" style="padding:10px;background:#222;color:#fff;border:none;border-radius:6px;cursor:pointer">Send</button></form></div></div><button id="rs-chat-btn" style="width:56px;height:56px;border-radius:50%;background:#222;color:#fff;border:none;cursor:pointer;font-size:1.5rem;box-shadow:0 6px 20px rgba(0,0,0,.25);float:right">&#128172;</button></div><script>(function(){var b=document.getElementById("rs-chat-btn");var p=document.getElementById("rs-chat-panel");if(b&&p){b.addEventListener("click",function(){p.style.display=p.style.display==="none"?"block":"none";});}})();</script>'},
  {label:'Events',tag:'section',html:'<section style="padding:70px 24px;max-width:820px;margin:0 auto"><h2 style="font-size:1.8rem;text-align:center;margin-bottom:36px">Upcoming Events</h2><div style="display:flex;flex-direction:column;gap:14px">{EVENTS}</div></section>'.replace('{EVENTS}',[['MAR 14:Spring Open House:6:00 PM - Main Location'],['MAR 22:Community Workshop:10:00 AM - Free to attend'],['APR 05:Members Night:7:00 PM - RSVP required']].map(function(e){var p=e[0].split(":");return '<div style="display:flex;gap:18px;align-items:center;padding:16px;border:1px solid #e5e5e5;border-radius:8px"><div style="text-align:center;flex-shrink:0;min-width:56px"><div style="font-size:.7rem;color:#c9a84c;font-weight:700;letter-spacing:1px">'+p[0]+'</div><div style="font-size:1.4rem;font-weight:800">'+p[1]+'</div></div><div style="flex:1"><h3 style="margin-bottom:4px">'+p[2]+'</h3><p style="color:#777;font-size:.9rem">'+p[3]+'</p></div><a href="#" style="padding:9px 18px;background:#222;color:#fff;text-decoration:none;border-radius:6px;font-size:.85rem;flex-shrink:0">RSVP</a></div>';}).join(''))},
  {label:'Intake Form',tag:'section',html:'<section style="padding:70px 24px;max-width:620px;margin:0 auto"><h2 style="font-size:1.8rem;text-align:center;margin-bottom:10px">New Client Intake</h2><p style="color:#666;text-align:center;margin-bottom:28px">Fill this out and we will be in touch to get started.</p><form action="mailto:you@example.com" method="post" enctype="text/plain" style="display:flex;flex-direction:column;gap:12px"><input name="name" placeholder="Full name" style="padding:12px;border:1px solid #ccc;border-radius:6px" required><input name="email" type="email" placeholder="Email" style="padding:12px;border:1px solid #ccc;border-radius:6px" required><input name="phone" placeholder="Phone" style="padding:12px;border:1px solid #ccc;border-radius:6px"><textarea name="project" placeholder="Tell us about your project or needs" rows="4" style="padding:12px;border:1px solid #ccc;border-radius:6px"></textarea><label style="display:flex;gap:8px;align-items:flex-start;font-size:.85rem;color:#555"><input type="checkbox" name="agree" required style="margin-top:3px">I agree to be contacted about my inquiry.</label><button type="submit" style="padding:14px;background:#222;color:#fff;border:none;border-radius:6px;cursor:pointer;font-weight:600">Submit</button></form></section>'},
  {label:'Coupon',tag:'section',html:'<section style="padding:56px 24px;text-align:center;background:#fff5f5"><div style="max-width:520px;margin:0 auto;border:2px dashed #d05050;border-radius:12px;padding:32px 24px;background:#fff"><div style="font-size:.8rem;letter-spacing:2px;color:#d05050;font-weight:700;text-transform:uppercase;margin-bottom:8px">Limited Time Offer</div><div style="font-size:2.4rem;font-weight:800;margin-bottom:8px">15% OFF</div><p style="color:#666;margin-bottom:16px">Your first service. Mention this code when you book.</p><div style="display:inline-block;background:#111;color:#fff;font-family:monospace;font-size:1.2rem;letter-spacing:3px;padding:10px 22px;border-radius:6px">SAVE15</div><p style="color:#999;font-size:.8rem;margin-top:14px">Expires end of month. Cannot be combined with other offers.</p></div></section>'},
  {label:'Gift Card',tag:'section',html:'<section style="padding:70px 24px;text-align:center"><h2 style="font-size:1.8rem;margin-bottom:28px">Gift Cards</h2><div style="max-width:420px;margin:0 auto;border-radius:14px;padding:32px;background:linear-gradient(135deg,#1a1a2e,#3a2a5e);color:#fff;text-align:left;box-shadow:0 10px 30px rgba(0,0,0,.2)"><div style="font-size:.75rem;letter-spacing:2px;opacity:.7;text-transform:uppercase;margin-bottom:24px">Gift Card</div><div style="font-size:2.2rem;font-weight:800;margin-bottom:6px">$50</div><div style="opacity:.8;font-size:.9rem">The perfect gift for anyone.</div></div><form action="mailto:you@example.com" method="post" enctype="text/plain" style="max-width:420px;margin:20px auto 0;display:flex;flex-direction:column;gap:10px"><input name="recipient" placeholder="Recipient email" style="padding:12px;border:1px solid #ccc;border-radius:6px"><button type="submit" style="padding:13px;background:#222;color:#fff;border:none;border-radius:6px;cursor:pointer;font-weight:600">Buy a Gift Card</button></form></section>'},
  {label:'Courses',tag:'section',html:'<section style="padding:70px 24px;max-width:960px;margin:0 auto"><h2 style="font-size:1.8rem;text-align:center;margin-bottom:36px">Online Courses</h2><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:24px">{COURSES}</div></section>'.replace('{COURSES}',[['Beginner Basics:6 lessons:$49'],['Pro Masterclass:12 lessons:$149'],['1-on-1 Coaching:Custom:$299']].map(function(c){var p=c[0].split(":");return '<div style="border:1px solid #e0e0e0;border-radius:10px;overflow:hidden"><div style="aspect-ratio:16/9;background:#eee"></div><div style="padding:18px"><h3 style="margin-bottom:4px">'+p[0]+'</h3><p style="color:#888;font-size:.85rem;margin-bottom:12px">'+p[1]+'</p><div style="display:flex;justify-content:space-between;align-items:center"><span style="font-weight:800;font-size:1.2rem">'+p[2]+'</span><a href="#" style="padding:8px 16px;background:#222;color:#fff;text-decoration:none;border-radius:6px;font-size:.85rem">Enroll</a></div></div></div>';}).join(''))},
  {label:'Portfolio',tag:'section',html:'<section style="padding:70px 24px;max-width:1000px;margin:0 auto"><h2 style="font-size:1.8rem;text-align:center;margin-bottom:36px">Our Work</h2><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:12px">{ITEMS}</div></section>'.replace('{ITEMS}',Array.from({length:6}).map(function(_,i){return '<div style="position:relative;aspect-ratio:4/3;background:#e6e6e6;border-radius:8px;overflow:hidden;display:flex;align-items:flex-end"><div style="width:100%;padding:12px;background:linear-gradient(transparent,rgba(0,0,0,.55));color:#fff;font-size:.85rem;font-weight:600">Project '+(i+1)+'</div></div>';}).join(''))},
];
...
::-webkit-scrollbar-thumb{background:rgba(201,168,76,.22);border:2px solid #0a0a12;border-radius:99px;}
::-webkit-scrollbar-thumb:hover{background:rgba(201,168,76,.4);}
html{scrollbar-color:rgba(201,168,76,.3) #0a0a12;}
...
.land-h1-dim{color:transparent;-webkit-text-stroke:1.4px rgba(201,168,76,.32);}
...
.land-feat{background:#07070e;padding:clamp(18px,3vw,32px) clamp(16px,2.5vw,28px);transition:background .3s var(--ease);min-width:0;overflow:hidden;}
.land-feat:hover{background:#0c0c1a;}
.land-feat-r{font-family:'Cinzel',serif;font-size:2rem;color:rgba(201,168,76,.4);margin-bottom:14px;line-height:1;transition:color .3s var(--ease),transform .3s var(--ease);}
.land-feat:hover .land-feat-r{transform:translateY(-2px);}
...
.land-cta-h{font-family:'Cinzel',serif;font-size:clamp(2rem,3.6vw,3.2rem);font-weight:900;letter-spacing:-.5px;text-wrap:balance;color:#ddd8ce;margin-bottom:14px;}
...
/* Bento capability grid — asymmetric tiles, the v2 "whole pipeline" beat */
.land-bento-wrap{padding:clamp(56px,8vw,104px) clamp(14px,4vw,60px);max-width:1280px;margin:0 auto;}
.land-bento-head{text-align:center;margin-bottom:44px;}
.land-bento-tag{font-family:'JetBrains Mono',monospace;font-size:.62rem;letter-spacing:2.5px;text-transform:uppercase;color:#c9a84c;margin-bottom:14px;}
.land-bento-head h2{font-family:'Cinzel',serif;font-weight:900;font-size:clamp(1.9rem,4vw,3rem);letter-spacing:-.5px;text-wrap:balance;color:#ddd8ce;}
.land-bento{display:grid;grid-template-columns:repeat(4,1fr);grid-auto-rows:170px;gap:14px;}
.land-b{border:1px solid rgba(201,168,76,.1);background:linear-gradient(180deg,#0c0c18,#090912);padding:24px;display:flex;flex-direction:column;justify-content:flex-end;position:relative;overflow:hidden;transition:transform .3s var(--ease),border-color .3s var(--ease);}
.land-b:hover{transform:translateY(-3px);border-color:rgba(201,168,76,.4);}
.land-b-ic{font-family:'Cinzel',serif;font-size:1.5rem;color:var(--gold);position:absolute;top:20px;left:24px;line-height:1;}
.land-b h3{font-family:'Cinzel',serif;font-weight:700;font-size:1.12rem;color:#ddd8ce;margin-bottom:7px;}
.land-b p{font-size:.84rem;font-weight:300;color:#5a5868;line-height:1.6;max-width:32ch;}
.land-b-wide{grid-column:span 2;}.land-b-tall{grid-row:span 2;}
.land-b-full{grid-column:span 4;flex-direction:row;align-items:flex-end;justify-content:space-between;gap:32px;flex-wrap:wrap;}
.land-b-full p{max-width:52ch;}
.land-b-big{font-family:'Cinzel',serif;font-weight:900;font-size:2.8rem;color:var(--gold);font-variant-numeric:tabular-nums;line-height:1;}
.land-b-cap{font-family:'JetBrains Mono',monospace;font-size:.6rem;letter-spacing:1.5px;text-transform:uppercase;color:#3a3848;margin-top:6px;}
.land-b-feat{background:linear-gradient(150deg,rgba(201,168,76,.14),#0a0a14 60%);}
@media(max-width:840px){.land-bento{grid-template-columns:repeat(2,1fr);}.land-b-full{grid-column:span 2;flex-direction:column;align-items:flex-start;gap:0;}}
@media(max-width:520px){.land-bento{grid-template-columns:1fr;}.land-b-wide,.land-b-full{grid-column:span 1;}.land-b-tall{grid-row:span 1;}}
...
.auth-card{width:min(380px,calc(100vw - 32px));background:#0d0d18;border:1px solid rgba(201,168,76,.14);padding:clamp(20px,5vw,36px);box-shadow:0 30px 80px rgba(0,0,0,.5);animation:modalIn .38s var(--ease);}
...
.inp{width:100%;background:#0a0a14;border:1px solid rgba(201,168,76,.1);color:#ddd8ce;font-family:'DM Sans',sans-serif;font-size:.85rem;padding:9px 13px;outline:none;transition:border-color .2s var(--ease),background .2s var(--ease),box-shadow .2s var(--ease);}
.inp:hover{border-color:rgba(201,168,76,.18);}
.inp:focus{border-color:rgba(201,168,76,.34);background:#0c0c18;box-shadow:0 0 0 3px rgba(201,168,76,.07);}
...
td{font-size:.8rem;font-weight:300;color:#9a96a2;padding:10px 12px;border-bottom:1px solid rgba(201,168,76,.04);vertical-align:middle;transition:background .2s var(--ease),color .2s var(--ease);}
tr:hover td{background:rgba(201,168,76,.045);color:#ddd8ce;cursor:pointer;}
...
.toast{background:#0d0d18;border:1px solid rgba(201,168,76,.14);padding:11px 16px;display:flex;align-items:center;gap:10px;min-width:240px;animation:toast-in .32s var(--ease);box-shadow:0 14px 40px rgba(0,0,0,.45);pointer-events:auto;}
...
.crm-f{font-family:'JetBrains Mono',monospace;font-size:.58rem;letter-spacing:1.5px;text-transform:uppercase;padding:5px 12px;cursor:pointer;background:#0a0a14;border:1px solid rgba(201,168,76,.06);color:#6a6878;transition:color .2s var(--ease),border-color .2s var(--ease),background .2s var(--ease);}
.crm-f:hover{color:#ddd8ce;border-color:rgba(201,168,76,.22);}
...
.dr-sb{font-family:'JetBrains Mono',monospace;font-size:.54rem;letter-spacing:1px;text-transform:uppercase;padding:6px 8px;cursor:pointer;background:#0a0a14;border:1px solid rgba(201,168,76,.07);color:#6a6878;transition:color .2s var(--ease),border-color .2s var(--ease),background .2s var(--ease);text-align:center;}
...
.tone-btn{background:#0a0a14;border:1px solid rgba(201,168,76,.07);padding:9px 10px;cursor:pointer;text-align:center;transition:border-color .2s var(--ease),background .2s var(--ease),transform .2s var(--ease);}
.tone-btn:active{transform:scale(.98);}
...
.app-card{background:#0d0d18;border:1px solid rgba(201,168,76,.08);padding:16px 18px;display:flex;align-items:center;gap:14px;transition:border-color .3s var(--ease),transform .3s var(--ease);}
.app-card:hover{border-color:rgba(201,168,76,.3);transform:translateY(-2px);}
...
.hamburger span{width:20px;height:2px;background:#c9a84c;display:block;transition:transform .2s var(--ease),opacity .2s var(--ease);}
...
.page-tab{font-family:'JetBrains Mono',monospace;font-size:.58rem;letter-spacing:1.5px;text-transform:uppercase;padding:7px 14px;background:#0a0a14;border:1px solid rgba(201,168,76,.08);color:#3a3848;cursor:pointer;transition:color .2s var(--ease),border-color .2s var(--ease),background .2s var(--ease);}
...
.mkt-filter-btn{display:block;width:100%;text-align:left;background:none;border:none;font-family:'JetBrains Mono',monospace;font-size:.6rem;letter-spacing:1px;text-transform:uppercase;color:#7a7888;padding:7px 10px;cursor:pointer;transition:color .2s var(--ease),background .2s var(--ease);}
.mkt-filter-btn:hover{color:#ddd8ce;background:rgba(201,168,76,.06);}
...
.search-inp{background:#0a0a14;border:1px solid rgba(201,168,76,.1);color:#ddd8ce;font-family:'JetBrains Mono',monospace;font-size:.62rem;padding:7px 14px;outline:none;width:220px;letter-spacing:1px;transition:width .3s var(--ease),border-color .2s var(--ease);}
...
.opt-btn{background:#0d0d18;border:1px solid rgba(201,168,76,.07);padding:8px 10px;cursor:pointer;text-align:center;transition:border-color .2s var(--ease),background .2s var(--ease),transform .2s var(--ease);}
...
function LandingPage({onSignIn,onGetStarted,user,onDashboard}) {
  ...
  return (
    <div className="land">
      ...
      {/* BENTO — asymmetric capability grid, the v2 "whole pipeline" beat */}
      <section className="land-bento-wrap reveal">
        <div className="land-bento-head">
          <div className="land-bento-tag">Everything in one place</div>
          <h2>The whole pipeline, built in.</h2>
        </div>
        <div className="land-bento">
          <div className="land-b land-b-tall land-b-feat">
            <div className="land-b-ic">ᚦ</div>
            <h3>Real prospects</h3>
            <p>Live local businesses from Google — scored on why they need you. Never invented.</p>
          </div>
          <div className="land-b land-b-wide">
            <h3>Pitches that land</h3>
            <p>Specific, tactical outreach written from the prospect's own data. Zero "unleash / seamless" filler.</p>
          </div>
          <div className="land-b">
            <div className="land-b-big">41</div>
            <p className="land-b-cap">Industry playbooks</p>
          </div>
          <div className="land-b land-b-wide">
            <div className="land-b-ic">ᚲ</div>
            <h3>Agent-built sites</h3>
            <p>Complete, semantic, animated HTML you fully own and export — held to a strict anti-slop bar.</p>
          </div>
          <div className="land-b">
            <div className="land-b-big">100%</div>
            <p className="land-b-cap">Your code</p>
          </div>
          <div className="land-b land-b-full">
            <div>
              <div className="land-b-ic">ᛏ</div>
              <h3>Agency OS + Marketplace</h3>
              <p>Invoices, contracts, CRM, and a UI/UX component database — buy, sell, or unlock by plan.</p>
            </div>
            <p className="land-b-cap">Proposals · E-signature · Stripe invoicing · Client health · Templates</p>
          </div>
        </div>
      </section>
...
function ScannerPage({onAdd,prospects,toast,setPage}){
  ...
  const[historySearchOpen,setHistorySearchOpen]=useState(null);
  const[historyResults,setHistoryResults]=useState([]);
  const[historyLoading,setHistoryLoading]=useState(false);
  const openHistorySearch=(entry)=>{
    setHistorySearchOpen(entry);
    setHistoryResults([]);
    scan({...entry,setResults:setHistoryResults,setLoading:setHistoryLoading,recordHistory:false});
  };
  ...
  useEffect(()=>{window.storage.get('rs3_recent_searches').then(r=>{if(r?.value)try{setRecentSearches(JSON.parse(r.value));}catch(e){}}).catch(()=>{});},[]);
  const logRecentSearch=(entry)=>{
    setRecentSearches(prev=>{
      const next=[{...entry,id:uid(),searchedAt:new Date().toISOString()},...prev].slice(0,25);
      window.storage.set('rs3_recent_searches',JSON.stringify(next)).catch(()=>{});
      return next;
    });
  };

  // Accepts optional overrides so this same logic can either run the main
  // scanner (writing to `results`/`loading`) or regenerate a past search
  // from Recent Searches into its own modal state, without the two
  // stepping on each other's state.
  const scan=async(opts)=>{
    const p=opts||{};
    const targetCity=p.city??city;
    const targetCat=p.cat??cat;
    const targetCount=p.count??count;
    const targetMinRating=p.minRating??minRating;
    const targetMinReviews=p.minReviews??minReviews;
    const targetKeyword=p.keyword??keyword;
    const setResultsFn=p.setResults||setResults;
    const setLoadingFn=p.setLoading||setLoading;
    ...
    try{
      ...
      if(placesKey){
        ...
        const query=`${businessType} in ${targetCity}`;
        const resp=await fetch('https://places.googleapis.com/v1/places:searchText',{
          method:'POST',
          headers:{
            'Content-Type':'application/json',
  ...
  const[watchedNames,setWatchedNames]=useState(new Set());
  useEffect(()=>{window.storage.get('rs3_watchlist').then(r=>{if(r?.value)try{setWatchedNames(new Set(JSON.parse(r.value).map(w=>w.name)));}catch(e){}}).catch(()=>{});},[]);
  const saveForLater=async(p)=>{
    if(watchedNames.has(p.name)){toast(`${p.name} is already saved.`,'info');return;}
    let list=[];
    try{const r=await window.storage.get('rs3_watchlist');if(r?.value)list=JSON.parse(r.value);}catch(e){}
    list.unshift({...p,id:uid(),savedAt:new Date().toISOString(),lastChecked:null,changes:null});
    await window.storage.set('rs3_watchlist',JSON.stringify(list)).catch(()=>{});
    setWatchedNames(prev=>new Set([...prev,p.name]));
    toast(`${p.name} saved for later.`,'success');
  };
  ...
  return(
    <div>
      ...
      <div style={{display:'flex',gap:0,marginBottom:12,border:'1px solid rgba(201,168,76,.12)'}}>
        {[{id:'city',label:'🔍 City Search'},{id:'business',label:'🏢 Business Lookup'},{id:'history',label:'🕐 Recent Searches'}].map(m=>(
          <button key={m.id} onClick={()=>{setScanMode(m.id);setNeedsDetail(false);}} style={{flex:1,padding:'10px 14px',background:scanMode===m.id?'rgba(201,168,76,.1)':'transparent',border:'none',borderRight:m.id!=='history'?'1px solid rgba(201,168,76,.12)':'none',cursor:'pointer',fontFamily:"'JetBrains Mono',monospace",fontSize:'.62rem',letterSpacing:'2px',textTransform:'uppercase',color:scanMode===m.id?'#c9a84c':'#3a3848',transition:'all .2s'}}>
            {m.label}
          </button>
        ))}
      </div>
      ...
      {scanMode==='history'&&(
        <div className="card" style={{marginBottom:12}}>
          <div className="card-sub" style={{marginBottom:12}}>Click a past search to re-run it fresh in a popup — this regenerates results, it doesn't restore the old ones.</div>
          {recentSearches.length===0?(
            <div className="empty"><div className="empty-sub">No searches yet. Run a scan and it'll show up here.</div></div>
          ):(
            <div style={{display:'flex',flexDirection:'column'}}>
              {recentSearches.map(s=>(
                <div key={s.id} style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:10,padding:'10px 4px',borderBottom:'1px solid rgba(201,168,76,.06)',cursor:'pointer'}} onClick={()=>openHistorySearch(s)}>
                  <div style={{display:'flex',alignItems:'center',gap:10,minWidth:0}}>
                    <span style={{fontFamily:"'Cinzel',serif",color:'#3a3848',fontSize:'1rem'}}>🕐</span>
                    <div style={{minWidth:0}}>
                      <div style={{fontSize:'.86rem',fontWeight:300,color:'#c9b87a',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{s.cat||s.keyword||'Any business'} <span style={{color:'#3a3848'}}>in</span> {s.city}</div>
                      <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:'.56rem',color:'#3a3848',letterSpacing:'1px',marginTop:2}}>{new Date(s.searchedAt).toLocaleString()} · {s.resultCount} results · {s.source==='places'?'Google Places':'AI generated'}</div>
                    </div>
                  </div>
                  <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:'.6rem',color:'#c9a84c',flexShrink:0}}>Re-run →</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
          ...
          <div className="field" style={{margin:0}}>
            ...
            <div style={{display:"flex",gap:6,marginTop:6}}>
              <select className="inp" style={{flex:1}} value={BUSINESS_TYPES.includes(cat)?cat:""} onChange={e=>e.target.value&&setCat(e.target.value)}>
                <option value="">— or pick from a list —</option>
                {BUSINESS_TYPES.map(t=><option key={t} value={t}>{t}</option>)}
              </select>
              <button type="button" className="btn btn-ghost btn-sm" title="Randomize business type and city" onClick={()=>{setCat(BUSINESS_TYPES[Math.floor(Math.random()*BUSINESS_TYPES.length)]);setCity(WORLD_CITIES[Math.floor(Math.random()*WORLD_CITIES.length)].name);}}>🎲 Surprise me</button>
            </div>
          </div>
        </div>
    ...
    {explainProspect&&<LeadScoreExplainer prospect={explainProspect} onClose={()=>setExplainProspect(null)}/>}
      ...
      {historySearchOpen&&(
        <div className="modal-bg" onClick={()=>setHistorySearchOpen(null)}>
          <div style={{background:"#0d0d18",border:"1px solid rgba(201,168,76,.2)",width:"min(1100px,94vw)",height:"88vh",display:"flex",flexDirection:"column"}} onClick={e=>e.stopPropagation()}>
            <div style={{padding:"16px 20px",borderBottom:"1px solid rgba(201,168,76,.08)",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
              <div>
                <div style={{fontFamily:"'Cinzel',serif",fontSize:"1.1rem",fontWeight:700,color:"#ddd8ce"}}>{historySearchOpen.cat||historySearchOpen.keyword||'Any business'} in {historySearchOpen.city}</div>
                <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:".56rem",letterSpacing:"2px",color:"#3a3848",textTransform:"uppercase",marginTop:3}}>Regenerated fresh — not the original saved results</div>
              </div>
              <button className="btn btn-ghost btn-sm" onClick={()=>setHistorySearchOpen(null)}>✕</button>
            </div>
            <div style={{flex:1,overflow:"auto",padding:20}}>
              {historyLoading?(
                <div style={{display:"flex",flexDirection:"column",alignItems:"center",padding:"48px",gap:14}}><Spinner lg/><div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:".62rem",letterSpacing:"2px",color:"#2e2d3c",textTransform:"uppercase"}}>Regenerating…</div></div>
              ):historyResults.length===0?(
                <div className="empty"><div className="empty-sub">No results came back this time — try again from City Search.</div></div>
              ):(
                <div className="pros-grid">
...
function WatchlistPage({toast,onAddToCRM,prospects,setPage}){
  ...
  const removeItem=(item)=>{persist(items.filter(it=>it.id!==item.id));toast(`Removed ${item.name}.`,'info');};
  const addToCRMFromWatchlist=(item)=>{
    if(prospects.some(x=>x.name?.toLowerCase()===item.name?.toLowerCase())){toast(`${item.name} is already in your CRM.`,'info');return;}
    onAddToCRM({...item,status:'Not Contacted',notes:item.notes||'',addedAt:now(),lastActivity:now()});
    toast(`${item.name} added to CRM.`,'success');
  };
  ...
  return(
    <div>
      ...
      {loading?(
        ...
        <div className="empty"><div className="empty-rune">💾</div><div className="empty-title">Nothing saved yet</div><div className="empty-sub">Find businesses in Prospect Scanner and hit "Save for Later" to build your watchlist.</div><button className="btn btn-gold btn-sm" onClick={()=>setPage('scanner')}>Go to Scanner →</button></div>
      ):(
              ...
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:10,flexWrap:'wrap'}}>
                <div>
                  <div style={{display:'flex',alignItems:'center',gap:8}}>
                    <div style={{fontFamily:"'Cinzel',serif",fontSize:'.92rem',fontWeight:700,color:'#ddd8ce'}}>{item.name}</div>
                    {checkingId===item.id&&<Spinner/>}
                    {item.changes?.length>0&&<span className="badge b-gold" style={{cursor:'pointer'}} onClick={()=>setExpandedId(expandedId===item.id?null:item.id)}>⚠ {item.changes.length} change{item.changes.length!==1?'s':''}</span>}
                  </div>
                  <div style={{fontSize:'.76rem',color:'#5a5868',marginTop:2}}>{item.phone} · {item.address}</div>
                  <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:'.56rem',color:'#3a3848',letterSpacing:'1px',marginTop:4}}>Saved {new Date(item.savedAt).toLocaleDateString()} {item.lastChecked?`· Checked ${new Date(item.lastChecked).toLocaleString()}`:''}</div>
                </div>
                <div style={{display:'flex',gap:6}}>
                  <button className="btn btn-gold btn-sm" onClick={()=>addToCRMFromWatchlist(item)}>+ Add to CRM</button>
                  <button className="btn btn-ghost btn-sm" onClick={()=>removeItem(item)}>Remove</button>
                </div>
              </div>
              {expandedId===item.id&&item.changes?.length>0&&(
                <div style={{marginTop:10,paddingTop:10,borderTop:'1px solid rgba(201,168,76,.08)',display:'flex',flexDirection:'column',gap:4}}>
                  {item.changes.map((c,i)=><div key={i} style={{fontSize:'.78rem',color:'#c9a84c'}}>{c}</div>)}
                </div>
              )}
            </div>
          ))}
...
function AgencyOSPage({prospects,proposals,addProposal,updateProposal,invoices,addInvoice,updateInvoice,toast}){
  ...
  // Automated client reporting: compiles a client's REAL data (their
  // proposals, invoices paid/outstanding, hours logged, fee) and has AI turn
  // it into a warm, client-facing monthly update. Numbers are real; only the
  // narrative is generated.
  const[report,setReport]=useState(null);
  const[reportLoadingId,setReportLoadingId]=useState('');
  const generateClientReport=async(c)=>{
    setReportLoadingId(c.id);
    try{
      const cProps=proposals.filter(p=>p.client===c.name);
      const cInvs=invoices.filter(i=>i.client===c.name);
      const paid=cInvs.filter(i=>i.status==='Paid').reduce((a,i)=>a+Number(i.total||0),0);
      const outstanding=cInvs.filter(i=>i.status!=='Paid').reduce((a,i)=>a+Number(i.total||0),0);
      const hours=(timeEntries||[]).filter(t=>t.client===c.name).reduce((a,t)=>a+(Number(t.secs||0)/3600),0);
      const fee=fees[c.id]||0;
      const facts=`Client: ${c.name} (${c.category||'business'} in ${c.city||'their area'}). Proposals sent: ${cProps.length}. Invoices: ${cInvs.length} total, $${paid.toFixed(0)} paid, $${outstanding.toFixed(0)} outstanding. Hours logged this period: ${hours.toFixed(1)}. Monthly fee: $${fee}.`;
      const raw=await callClaude(`Write a warm, professional monthly client update from a web design agency to their client, using ONLY these real figures (do not invent numbers): ${facts} Structure: a friendly greeting, a short "what we worked on" summary, a plain-English financial recap, and 2-3 clear next steps. Keep it concise and client-facing.`,900);
      setReport({client:c.name, content:raw.trim()});
    }catch(e){toast('Report generation failed — try again.','error');}
    setReportLoadingId('');
  };

  // Contracts tab (was a blank tab): AI drafts a plain-language web design
  // service agreement for a selected client.
  const[contractClient,setContractClient]=useState('');
  const[contract,setContract]=useState('');
  const[contractLoading,setContractLoading]=useState(false);
  const generateContract=async()=>{
    const c=prospects.find(p=>p.id===contractClient);
    if(!c){toast('Select a client first.','error');return;}
    setContractLoading(true);setContract('');
    try{
      const raw=await callClaude(`Draft a clear, professional web design service agreement between a web design agency and the client "${c.name}" (${c.category||'business'}${c.city?` in ${c.city}`:''}). Include: the parties, scope of work, deliverables, timeline, payment terms, a revisions policy, ownership/IP transfer on final payment, a cancellation clause, and a signature block. Use plain, fair, readable language — not dense legalese. Start with a one-line note that this is a template to review with a lawyer before use.`,1800);
      setContract(raw.trim());
    }catch(e){toast('Contract generation failed — try again.','error');}
...
const TOOLS=[{id:"clients",r:"ᚠ",l:"Clients"},{id:"proposals",r:"ᚨ",l:"Proposals"},{id:"contracts",r:"ᚦ",l:"Contracts"},{id:"invoices",r:"ᛊ",l:"Invoices"},{id:"revenue",r:"ᛟ",l:"Revenue"},{id:"onboarding",r:"ᛈ",l:"Onboarding"}];
  return(
    <div>
      ...
      <div className="agency-layout">
        ...
        <div className="agency-content">
          {tab==="clients"&&(<><div className="sh"><div><div className="sh-title">Active Clients</div><div className="sh-sub">{clients.length} clients</div></div></div>{clients.length===0?<div className="empty"><div className="empty-rune">ᚠ</div><div className="empty-title">No clients yet</div><div className="empty-sub">Mark prospects as Closed in the CRM to add them here.</div></div>:<div className="client-grid">{clients.map(c=><div key={c.id} className="client-card"><div className="client-card-head"><div className="client-name">{c.name}</div><span className="badge b-green">Active</span></div>
              ...
              </div><div className="client-meta">{c.category} · {c.city}</div><div className="client-fee-row"><div><div className="client-fee-label">Monthly Fee</div><div className="client-fee">${fees[c.id]||0}/mo</div></div><input className="inp" style={{width:90,marginLeft:"auto"}} placeholder="$/mo" type="number" value={fees[c.id]||""} onChange={e=>setFees(prev=>({...prev,[c.id]:e.target.value}))}/></div><div style={{display:"flex",gap:6,marginTop:4,flexWrap:'wrap'}}><button className="btn btn-ghost btn-xs" onClick={()=>{setInvClient(c.id);setShowInvModal(true);}}>+ Invoice</button><button className="btn btn-ghost btn-xs" disabled={reportLoadingId===c.id} onClick={()=>generateClientReport(c)}>{reportLoadingId===c.id?'…':'📊 Report'}</button><button className="btn btn-gold btn-xs" onClick={()=>{setSelProspect(c.id);setTab("proposals");}}>Generate Proposal</button></div></div>)}</div>}</>)}
          ...
          {tab==="contracts"&&(<><div className="sh"><div><div className="sh-title">Contracts</div><div className="sh-sub">Generate a client service agreement</div></div></div><div className="card" style={{marginBottom:16,padding:16}}><div style={{display:"flex",gap:10,alignItems:"flex-end"}}><div className="field" style={{margin:0,flex:1}}><label>Select Client</label><select className="inp" value={contractClient} onChange={e=>setContractClient(e.target.value)}><option value="">— Choose —</option>{prospects.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></div><button className="btn btn-gold" onClick={generateContract} disabled={contractLoading||!contractClient} style={{height:38}}>{contractLoading?<><Spinner/>Drafting…</>:"Generate →"}</button></div>{contract&&<div style={{marginTop:14,background:"#0a0a14",border:"1px solid rgba(201,168,76,.08)",padding:16}}><pre style={{fontFamily:"'DM Sans',sans-serif",fontSize:".84rem",fontWeight:300,color:"#c9b87a",lineHeight:1.85,whiteSpace:"pre-wrap"}}>{contract}</pre><div style={{display:"flex",justifyContent:"flex-end",gap:8,marginTop:10}}><button className="btn btn-ghost btn-sm" onClick={()=>{navigator.clipboard.writeText(contract);toast("Contract copied.","success");}}>Copy</button><button className="btn btn-ghost btn-sm" onClick={()=>{const w=window.open("","_blank");if(!w)return;w.document.write('<html><body style="font-family:sans-serif;padding:48px;max-width:720px;margin:0 auto;line-height:1.8;color:#111"><pre style="white-space:pre-wrap;font-family:sans-serif">'+contract.replace(/</g,"&lt;")+'</pre></body></html>');w.document.close();w.print();}}>Print PDF</button></div></div>}</div><div style={{fontSize:".72rem",color:"#3a3848",lineHeight:1.6}}>Generated agreements are templates. Have a lawyer review before sending to a client.</div></>)}
          {tab==="onboarding"&&(<><div className="sh"><div><div className="sh-title">Client Onboarding</div><div className="sh-sub">Your repeatable new-client checklist</div></div></div><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))",gap:12}}>{[{t:"Welcome & Kickoff",items:["Send the welcome email (AI Studio → Client Welcome)","Book the kickoff call","Confirm scope, timeline, and point of contact"]},{t:"Gather Assets",items:["Logo, brand colors, and fonts","Photos and existing copy","Domain and hosting/login access","Social links and business hours"]},{t:"Agreement & Payment",items:["Send the service agreement (Contracts tab)","Collect the signed contract","Send the deposit invoice (Invoices tab)","Confirm payment received"]},{t:"Build & Launch",items:["Share a first preview link","Collect one round of feedback","Final review and approval","Deploy, then send the launch report (client card → Report)"]}].map((g,i)=>(<div key={i} className="card" style={{padding:16}}><div style={{fontFamily:"'Cinzel',serif",fontSize:".92rem",fontWeight:700,color:"#c9a84c",marginBottom:10}}>{g.t}</div><div style={{display:"flex",flexDirection:"column",gap:7}}>{g.items.map((it,j)=>(<label key={j} style={{display:"flex",gap:8,alignItems:"flex-start",fontSize:".8rem",fontWeight:300,color:"#9a96a2",lineHeight:1.5,cursor:"pointer"}}><input type="checkbox" style={{marginTop:3,cursor:"pointer"}}/>{it}</label>))}</div></div>))}</div></>)}
        </div>
      </div>
          {tab==='transcript'&&(
            <>
      ...
      {report&&<div className="modal-bg" onClick={e=>e.target.className==="modal-bg"&&setReport(null)}><div style={{background:"#0d0d18",border:"1px solid rgba(201,168,76,.2)",width:"min(640px,94vw)",maxHeight:"88vh",display:"flex",flexDirection:"column"}} onClick={e=>e.stopPropagation()}><div style={{padding:"16px 20px",borderBottom:"1px solid rgba(201,168,76,.08)",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}><div><div style={{fontFamily:"'Cinzel',serif",fontSize:"1rem",fontWeight:700,color:"#ddd8ce"}}>Client Report — {report.client}</div><div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:".56rem",letterSpacing:"2px",color:"#3a3848",textTransform:"uppercase",marginTop:3}}>Figures are real · narrative is AI-drafted</div></div><button className="btn btn-ghost btn-sm" onClick={()=>setReport(null)}>✕</button></div><div style={{flex:1,overflow:"auto",padding:20}}><pre style={{fontFamily:"'DM Sans',sans-serif",fontSize:".88rem",fontWeight:300,color:"#c9b87a",lineHeight:1.9,whiteSpace:"pre-wrap"}}>{report.content}</pre></div><div style={{padding:"12px 20px",borderTop:"1px solid rgba(201,168,76,.08)",display:"flex",gap:8,flexShrink:0}}><button className="btn btn-ghost btn-sm" onClick={()=>{navigator.clipboard.writeText(report.content);toast("Report copied.","success");}}>Copy</button><button className="btn btn-gold btn-sm" onClick={async()=>{const email=window.prompt(`Send report to ${report.client}. Enter their email:`);if(!email)return;try{await sendEmail({to:email,subject:`Your monthly update — ${report.client}`,html:`<div style="font-family:sans-serif;line-height:1.7">${report.content.replace(/\n/g,"<br/>")}</div>`});toast("Report emailed!","success");}catch(e){toast("Send failed: "+e.message,"error");}}}>✉️ Email to Client</button></div></div></div>}
    </div>
  );
}
...
function AIStudioPage({prospects,toast}){
  ...
  const generate=async()=>{
    ...
    try{
      ...
      else if(tool==="schema")prompt=`Generate copy-paste-ready JSON-LD schema markup for ${biz} ${customInput?`Extra details: ${customInput}.`:''} Provide: 1) LocalBusiness schema (name, address, phone, hours, geo, priceRange), 2) an FAQPage schema with 3-4 relevant Q&As, 3) a Review/AggregateRating snippet. Then add a short "AEO tips" section: 2-3 quick wins to make this business more likely to be cited by AI search assistants. Return the JSON-LD in clearly separated code blocks.`;
...
function MarketplacePage({toast,user}){
  ...
  const[newTmpl,setNewTmpl]=useState({name:"",cat:"",desc:"",price:"",html:"",layout:"",style_tags:"",asset_type:"template",license:"",source_url:"",attribution:"",attest:false});
  // Redistribution-permitting licenses only — mirrors the DB allow-list in
  // SUPABASE_FINAL.sql §9. Proprietary/third-party work has no valid option, so
  // it can't be listed. See COPYRIGHT_POLICY.md.
  const LICENSES=[{v:"original",l:"Original — I created this"},{v:"CC0",l:"CC0 (public domain)"},{v:"CC-BY",l:"CC-BY (attribution required)"},{v:"CC-BY-SA",l:"CC-BY-SA (attribution + share-alike)"},{v:"MIT",l:"MIT"},{v:"apache-2.0",l:"Apache-2.0"},{v:"BSD-3-Clause",l:"BSD-3-Clause"}];
  ...
  const submitTemplate=async()=>{
    ...
    if(!user?.id){toast('Please sign in first.','error');return;}
    // Copyright guardrail (COPYRIGHT_POLICY.md): a redistributable license is
    // mandatory, CC-BY family needs attribution, and the creator must attest.
    if(!newTmpl.license){toast("Choose a license — we can only list original or redistribution-permitted work.","error");return;}
    if((newTmpl.license==="CC-BY"||newTmpl.license==="CC-BY-SA")&&!newTmpl.attribution.trim()){toast("That license requires an attribution/credit line.","error");return;}
    if(!newTmpl.attest){toast("Please confirm this is your work or properly licensed.","error");return;}
    setSubmitting(true);
    try{
      ...
      const{data:inserted,error}=await sb.from('templates').insert({
        ...
        asset_type:newTmpl.asset_type||'template',license:newTmpl.license,
        license_source_url:newTmpl.source_url||null,attribution:newTmpl.attribution||null,
      }).select('id').single();
      ...
      setNewTmpl({name:"",cat:"",desc:"",price:"",html:"",layout:"",style_tags:"",asset_type:"template",license:"",source_url:"",attribution:"",attest:false});
      toast((newTmpl.asset_type==='component'?'Component':'Template')+" submitted — live in Browse now.","success");
      loadMarketplaceData();
    }catch(e){
  ...
  return(
    <div>
      ...
      {tab==="sell"&&<div className="sell-form"><div className="card-title" style={{marginBottom:4}}>Submit a Template or Component</div><div className="card-sub" style={{marginBottom:16}}>You earn 70% of every sale — paid out manually for now, see Earnings tab. Original or redistribution-licensed work only.</div>{loadError&&<div style={{fontSize:'.78rem',color:'#e07878',padding:'10px 14px',background:'rgba(224,120,120,.08)',border:'1px solid rgba(224,120,120,.2)',marginBottom:12}}>{loadError}</div>}<div className="field"><label>Template Name</label><input className="inp" placeholder="ProContractor" value={newTmpl.name} onChange={e=>setNewTmpl(p=>({...p,name:e.target.value}))}/></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><div className="field"><label>Category</label><select className="inp" value={newTmpl.cat} onChange={e=>setNewTmpl(p=>({...p,cat:e.target.value}))}><option value="">— Select —</option>{CATS.filter(c=>c!=="All").map(c=><option key={c} value={c}>{c}</option>)}</select></div><div className="field"><label>Price ($)</label><input className="inp" type="number" placeholder="49" value={newTmpl.price} onChange={e=>setNewTmpl(p=>({...p,price:e.target.value}))}/></div></div><div className="field"><label>Description</label><textarea className="inp" rows={2} value={newTmpl.desc} onChange={e=>setNewTmpl(p=>({...p,desc:e.target.value}))}/></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><div className="field"><label>Layout style</label><input className="inp" placeholder="e.g. hero-split, centered, full-bleed" value={newTmpl.layout} onChange={e=>setNewTmpl(p=>({...p,layout:e.target.value}))}/></div><div className="field"><label>Style tags (comma-separated)</label><input className="inp" placeholder="minimal, bold, dark, elegant" value={newTmpl.style_tags} onChange={e=>setNewTmpl(p=>({...p,style_tags:e.target.value}))}/></div></div><div style={{fontSize:'.68rem',color:'#3a3848',lineHeight:1.6,marginBottom:10,marginTop:-4}}>Layout + tags help the AI match your template to the right business. They power template selection, not just search.</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><div className="field"><label>Asset type</label><select className="inp" value={newTmpl.asset_type} onChange={e=>setNewTmpl(p=>({...p,asset_type:e.target.value}))}><option value="template">Full template (whole site)</option><option value="component">Component (mini-feature)</option></select></div><div className="field"><label>License</label><select className="inp" value={newTmpl.license} onChange={e=>setNewTmpl(p=>({...p,license:e.target.value}))}><option value="">— Select a license —</option>{LICENSES.map(l=><option key={l.v} value={l.v}>{l.l}</option>)}</select></div></div>{(newTmpl.license==="CC-BY"||newTmpl.license==="CC-BY-SA")&&<div className="field"><label>Attribution / credit line (required for this license)</label><input className="inp" placeholder="e.g. Design by Jane Doe, CC-BY 4.0" value={newTmpl.attribution} onChange={e=>setNewTmpl(p=>({...p,attribution:e.target.value}))}/></div>}{newTmpl.license&&newTmpl.license!=="original"&&<div className="field"><label>Source URL (optional — provenance)</label><input className="inp" placeholder="https://… where this is published/licensed" value={newTmpl.source_url} onChange={e=>setNewTmpl(p=>({...p,source_url:e.target.value}))}/></div>}<label style={{display:"flex",gap:8,alignItems:"flex-start",fontSize:".76rem",color:"#8a8898",lineHeight:1.6,margin:"4px 0 12px"}}><input type="checkbox" checked={newTmpl.attest} onChange={e=>setNewTmpl(p=>({...p,attest:e.target.checked}))} style={{marginTop:3,flexShrink:0}}/><span>I confirm this is my own work, or is licensed to permit resale. Rune Script does not resell work that is not yours.</span></label><div className="field"><label>HTML Code</label><textarea className="inp" rows={4} placeholder="Paste your full HTML here…" value={newTmpl.html} onChange={e=>setNewTmpl(p=>({...p,html:e.target.value}))}/></div><div style={{fontSize:'.68rem',color:'#3a3848',lineHeight:1.6,marginBottom:12}}>Your HTML is stored privately and only served to buyers/unlocked users. Previews run in an isolated sandbox, so your source is never exposed to non-buyers.</div><button className="btn btn-gold btn-full" onClick={submitTemplate} disabled={submitting}>{submitting?<><Spinner/>Submitting…</>:'Submit Template'}</button></div>}
...
function MobileNav({page, setPage, user, onLogout}) {
  ...
  const nav=[
    ...
    {id:'watchlist',r:'ᚱ',l:'Saved for Later'},
...
function CityPicker({value, onChange, placeholder}) {
  ...
  // query only initialized from value on mount above — without this, setting
  // city externally (example prompts, saved searches) updates the actual
  // city state used by scan() but the visible input text never changes,
  // making it look like only the business type got filled in.
...
function CommandPalette({setPage, prospects, onClose, toast}) {
  ...
  const ACTIONS = [
    ...
    {label:'Saved for Later',icon:'💾',action:()=>setPage('watchlist')},
...
export default function RuneScript(){
  ...
  const PAGES={
    ...
    watchlist:<WatchlistPage toast={toast} onAddToCRM={addProspect} prospects={prospects} setPage={setPage}/>,
  };
